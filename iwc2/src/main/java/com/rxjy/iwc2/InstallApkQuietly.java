...
/**
 * Created by 解亚鑫 on 2018/6/13.
 */

public class InstallApkQuietly {
    public static Context mContext = null;
    public static InstallApkQuietly mInstallApkQuietly = null;
    public static String sdRootPath = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "Download" + File.separator + "iwc2-debug.apk";//系统APP路径
    public static String sdPath = "/sdcard/update/updata.apk";//下载SD卡路径
    public static String packageAppName = "com.rxjy.iwc2";
    ...
    public static File downLoadFile(String httpUrl) {
        ...
        // TODO Auto-generated method stub
        final String fileName = "iwc2-debug.apk";
        File tmpFile = new File(mTempPath);
        if (!tmpFile.exists()) {
            tmpFile.mkdir();
        }
        final File file = new File(mTempPath + fileName);
    ...
    public static boolean isHasfile() {
        try {
            File f = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "Download"+File.separator+"iwc2-debug.apk");
            if (!f.exists()) {
                return false;
            }
        } catch (Exception e) {
    ...
    // 移动到system app目录�?
    public static void moveToRoot(String url) {
